...
from torch import optim as py_optim
import math
...
args.best_record = {'epoch': 181, 'iter': 0, 'val_loss': 1e10, 'acc': 0,
                    'acc_cls': 0, 'mean_iu': 0.73863, 'fwavacc': 0}
...
def main():
    ...
    if args.snapshot:
        net, optim = optimizer.load_weights(net, optim,
                               args.snapshot, args.restore_optimizer)
        lambda1 = lambda epoch: math.pow(1 - epoch / args.max_epoch, args.poly_exp)
        scheduler = py_optim.lr_scheduler.LambdaLR(optim, lr_lambda=lambda1, last_epoch=args.start_epoch-1)
        # print(scheduler)
        # print(optim)
        # exit(0)
    if args.evaluateF:
    ...
    # flops_count, params_count = get_model_complexity_info(net, (1024, 2048))
...
def train(train_loader, net, optim, curr_epoch, writer):
    ...
    for i, data in enumerate(train_loader):
        ...
        if args.joint_edgeseg_loss:
            ...
            inputs, gts, boundary_gt, _img_name = data

        batch_pixel_size = inputs.size(0) * inputs.size(2) * inputs.size(3)

        inputs, gts, boundary_gt = inputs.cuda(), gts.cuda(), boundary_gt.cuda()
        optim.zero_grad()
        if args.joint_edgeseg_loss:
            ...
            main_loss = net(inputs, gts=[gts, boundary_gt])
...
def validate(val_loader, net, criterion, optim, curr_epoch, writer):
    ...
    for val_idx, data in enumerate(val_loader):
        inputs, gt_image, boundary_gt, img_names = data
        assert len(inputs.size()) == 4 and len(gt_image.size()) == 3
        assert inputs.size()[2:] == gt_image.size()[1:]

        batch_pixel_size = inputs.size(0) * inputs.size(2) * inputs.size(3)
        inputs, gt_cuda, boundary_gt = inputs.cuda(), gt_image.cuda(), boundary_gt.cuda()

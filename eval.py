...
from datasets import cityscapes, kitti, pascal_context
...
parser.add_argument('--crop_size', type=int, default=456)
...
def inference_my(model, img, scales):
    w, h = img.size
...
def module_inference(model, image, flip=True):
    with torch.no_grad():
        output = model(image)
        prod = F.softmax(output, 1)
        if flip:
            fimg = flip_image(image)
            foutput = model(fimg)
            prod += F.softmax(flip_image(foutput), 1)
        prod = torch.exp(prod)
    return prod
...
def inference_zhang(model, img, scales):
    img_transform = transforms.Compose(
        [transforms.ToTensor(), transforms.Normalize(*mean_std)])
    ...
    with torch.cuda.device_of(img):
        scores = img.new().resize_(batch, args.dataset_cls.num_classes, h, w).zero_().cuda()
    for scale in scales:
        long_size = int(math.ceil(args.base_size * scale))
        if h > w:
            height = long_size
        ...
        cur_img = resize_image(img, height, width)
        if long_size <= crop_size:
            pad_img = pad_image(cur_img, mean_std[0],
                                mean_std[1], crop_size)
            outputs = module_inference(model, pad_img)
            ...
            with torch.cuda.device_of(img):
                outputs = img.new().resize_(batch, args.dataset_cls.num_classes, ph, pw).zero_().cuda()
                count_norm = img.new().resize_(batch, 1, ph, pw).zero_().cuda()
            # grid evaluation
            for idh in range(h_grids):
                for idw in range(w_grids):
                    ...
                    # pad if needed
                    pad_crop_img = pad_image(crop_img, mean_std[0],
                                             mean_std[1], crop_size)
                    output = module_inference(model, pad_crop_img)
...
def inference_crop(model, img, label, scales):
    ...
    probs = torch.zeros((N, args.dataset_cls.num_classes, H, W))
    probs.requires_grad = False
    for sc in scales:
        if sc < 1:
            prob = eval_chip(model, img)
        else:
            prob = scale_crop_eval(model, img, sc)
...
def compute_hist(pred, lb):
    n_classes = args.dataset_cls.num_classes
    ignore_idx = args.dataset_cls.ignore_label
...
def scale_crop_eval(mdoel, im, scale):
    N, C, H, W = im.size()
    new_hw = [int(H * scale), int(W * scale)]
    im = F.interpolate(im, new_hw, mode='bilinear', align_corners=True)
    prob = crop_eval(mdoel, im)
    prob = F.interpolate(prob, (H, W), mode='bilinear', align_corners=True)
    return prob


def crop_eval(model, img):
    cropsize = args.crop_size
    stride_rate = 2. / 3.
    N, C, H, W = img.size()
    long_size, short_size = (H, W) if H > W else (W, H)
    if long_size < cropsize:
        im, indices = pad_tensor(img, (cropsize, cropsize))
        prob = eval_chip(model, im)
        ...
        if short_size < cropsize:
            if H < W:
                im, indices = pad_tensor(img, (cropsize, W))
            else:
                im, indices = pad_tensor(img, (H, cropsize))
        N, C, H, W = img.size()
        n_x = math.ceil((W - cropsize) / stride) + 1
        n_y = math.ceil((H - cropsize) / stride) + 1
        prob = torch.zeros(N, args.dataset_cls.num_classes, H, W).cuda()
        prob.requires_grad = False
        for iy in range(n_y):
            for ix in range(n_x):
                hed, wed = min(H, stride * iy + cropsize), min(W, stride * ix + cropsize)
                hst, wst = hed - cropsize, wed - cropsize
                chip = img[:, :, hst:hed, wst:wed]
                prob_chip = eval_chip(model, chip)
...
def inference_whole(model, img, scales):
    ...
    for scale in scales:
        ...
        for flip in range(flip_range):
            ...
            with torch.no_grad():
                input = image.unsqueeze(0).cuda()
                scale_out = model(input, img_names)
                scale_out = F.upsample(scale_out, size=(origh, origw), mode="bilinear", align_corners=True)
                scale_out = scale_out.squeeze().cpu().numpy()
                if flip:
                    scale_out = scale_out[:, :, ::-1]
...
def setup_loader():
    ...
    if args.dataset == 'cityscapes':
        ...
        args.dataset_cls = pascal_context
        print(args.mode, args.split)
        test_set = args.dataset_cls.PASCAL_CONTEXT("semantic", args.split,
                                                   transform=val_input_transform,
                                                   target_transform=target_transform,
                                                   cv_split=args.cv_split
                                                   )
...
class RunEval():
    ...
    def inf(self, imgs, img_names, gt, inference, net, scales, pbar, base_img):
        ...
        if self.inference_mode == 'pooling':
            ...
            img = to_pil(imgs[0])
        # prediction = inference_crop(model=net, img=img, label=gt, scales=scales)[0]
        prediction = inference_zhang(net, img, scales)[0]
...
def main():
    ...
    elif args.inference_mode == 'my':
        inference = inference_my
    else:
